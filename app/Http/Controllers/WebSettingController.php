...
class WebSettingController extends Controller
{
    ...
    public function putGeneralSetting(Request $request, $id)
    {
        ...
        try {
            ...
            $general = $request->except('_method', '_token');
            if($request->hasFile('image')){
                $image = $request->file('image');
                $filename = "logo".'.'.$image->getClientOriginalExtension();
                $location = 'assets/images/' . $filename;
                Image::make($image)->resize(200,70)->save($location);
    ...
    public function postSlider(Request $request)
    {
        ...
        $ge = $request->except('_method','_token');
        if($request->hasFile('image')){
            $image = $request->file('image');
    ...
    public function getAnnouncementCreate()
    {
        ...
        return view('websetting.announcement-create',$data);
    }
    public function postAnnouncementCreate(Request $request)
    {
//        dd($request);
//        try {
//            $menu = Input::except('_method','_token');
            ...
            $ann->save();
            session()->flash('message', 'Announcement Create Successfully.');
            Session::flash('type', 'success');
            return redirect()->back();
//        } catch (\PDOException $e) {
//            session()->flash('message', 'Some Problem Occurs, Please Try Again!');
...
//        }
    ...
    public function showAnnouncementCreate()
    {
        $data['site_title'] = $this->site_title;
        $data['page_title'] = "Show All Announcements";
        $data['general'] = GeneralSetting::first();
        $data['basic'] = BasicSetting::first();
        $data['menu'] = Announcement::all();
        return view('websetting.announcement-show',$data);
    }
    public function editAnnouncementCreate($id)
    {
        $data['site_title'] = $this->site_title;
        $data['general'] = GeneralSetting::first();
        $data['basic'] = BasicSetting::first();
        $data['page_title'] = "Edit Announcement";
        $data['menu'] = Announcement::findorFail($id);
        return view('websetting.announcement-edit',$data);
    }
    public function updateAnnouncementCreate(Request $request,$id)
    {
        $menus = Announcement::findOrFail($id);
        $this->validate($request,[
            'title' =>'required|unique:announcements,title,'.$menus->id,
            'description' => 'required'
        ]);
        try {
            $menu = $request->except('_method','_token');
            $menus->fill($menu)->save();
    ...
    public function deleteAnnouncementCreate(Request $request)
    {
        try {
            $menu = Announcement::findOrFail($request->id);
            $menu->delete();
            session()->flash('message', 'Announcement Deleted Successfully.');
            Session::flash('type', 'success');
            return redirect()->back();
        } catch (\PDOException $e) {
            session()->flash('message', 'Some Problem Occurred, Please Try Again!');
            Session::flash('type', 'danger');
            return redirect()->back();
        }
    }
