...
class PaymentController extends Controller
{
    ...
    public function depositConfirm()
    {
        ...
        if(is_null($data))
        {
            session()->flash('message','Invalid Deposit Request');
        ...
        elseif ($data->payment_type == 501)
        {
            ...
            if($data->btc_amo==0 || $data->btc_acc=="")
            {
                ...
                $mysite_root = url('/');
                $secret = "ABIR";
//                $my_xpub ='xpub6ByYjkvodNWAWgjomrMUyKktXHDEHVdVfGrFvNtZSEa69KSHEUzubX3Ak3G6Ts1FLRZ7eXKWqxnSN223oDKBqDqoK5KXTSmpzBBwDaJFEYo';             //$gatewayData->val2;
                $my_xpub =$gatewayData->val2;             //$gatewayData->val2;
                $my_api_key = $gatewayData->val1;

                $invoice_id = $track;
                $callback_url = $mysite_root . "/ipnbtc?invoice_id=" . $invoice_id . "&secret=" . $secret;

                $resp = @file_get_contents($blockchain_receive_root . "v2/receive?xpub=" . $my_xpub. "&callback=" . urlencode($callback_url) ."&key=" . $my_api_key);
            ...
            $bitcoin['code'] =  "<img src=\"https://blockchain.info/qr?data=".$DepositData->btc_acc."&size=250\" title='' style='width:250px;' />";
    ...
    public function ipnBchain()
    {
        ...
        if ($data->status==0)
        {
            if($data->custom==$track && $data->btc_acc==$address && $secret=="ABIR" && $confirmations>3)
            {
                $data->btc_amo= $value_in_btc;
                //Update User Data
                $this->userDataUpdate($data);
    ...
    public function btchck()
    {
        $btc_acc =$_GET['q'];
        $data = PaymentLog::where('btc_acc',$btc_acc)->where('status',1)->first();
        if ($data)
        {
            return $data->status;
        }else{
